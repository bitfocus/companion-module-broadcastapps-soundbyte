...
class SoundByteInstance extends InstanceBase {
	...
	async destroy() {
		[this.pollInterval, this.soundsCheckInterval, this.connectionCheckInterval].forEach(interval => {
	...
	haveSoundsChanged(newSounds) {
		...
		return newSounds.some(newSound => {
			...
			return !existing || existing.name !== newSound.name || 
	...
	async pollSoundStates() {
		...
		try {
			...
			for (const sound of this.status.sounds) {
				try {
					const isPlaying = (await this.http.get(`/api/status/${sound.id}`)).data.isPlaying || false
	...
	updateVariableDefinitions() {
		const variables = [
			...
			...this.status.sounds.map(sound => ({ name: `${sound.name} Name`, variableId: `sound_${sound.id}_name` }))
		]
	...
	updateVariableValues() {
		...
		this.setVariableValues({
			...
			...Object.fromEntries(this.status.sounds.map(sound => [`sound_${sound.id}_name`, sound.shortName || sound.name]))
		})
	}
	...
	async stopAllSounds() {
		try {
			...
			if (response.data.success) {
				this.status.playingStates.forEach((_, soundId) => this.status.playingStates.set(soundId, false))
				this.updateVariableValues()
				this.checkFeedbacks()
			} else {
